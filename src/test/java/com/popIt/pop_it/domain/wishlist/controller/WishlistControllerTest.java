...
class WishlistControllerTest {
    ...
    void toggle_add_success() throws Exception {
        mockMvc.perform(post("/api/v1/spaces/{spaceId}/wishlists", space.getId())
    ...
    void toggle_spaceNotFound() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/spaces/{spaceId}/wishlists", notExistSpaceId)
                        .header("Authorization", accessToken))
    ...
    void toggle_deletedSpace() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/spaces/{spaceId}/wishlists", deleted.getId())
                        .header("Authorization", accessToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("SPACE404_1"));
