...
public class SpaceController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    - imageUrls: presigned URL 발급(POST /api/v1/uploads/presigned-url)으로 S3에 직접 업로드한 뒤 받은 fileUrl 목록입니다.
    ...
    @Operation(
            ...
            description = """
                    ...
                    [startDate, endDate] 날짜 필터 캘린더의 기간 선택에 대응합니다. yyyy-MM-dd 형식이며 종료일을 포함합니다.
                      선택한 기간 '전체'가 비어 있는 공간만 조회합니다. 즉,
                      - 호스트가 설정한 계약 가능 기간이 선택 기간을 모두 포함하고,
                      - 선택 기간과 하루라도 겹치는 예약(취소 제외, 승인 대기 포함)이 없는 공간만 내려갑니다.
                      두 값은 반드시 함께 전달해야 하며(한쪽만 보내면 400), 둘 다 생략하면 기간 조건 없이 조회합니다.
